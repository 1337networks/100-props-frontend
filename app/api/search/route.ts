...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Search Pinecone - retrieve a wider set, then filter/rank tightly.
    // This reduces false negatives like Day 51 (Rat Trap) missing on queries like "compare".
    const matches = await searchPinecone(embedding, 80)
    ...
    // If no highly relevant matches, show top with lower threshold
    const FALLBACK_SCORE = 0.25
    const finalMatches = relevantMatches.length > 0
      ? relevantMatches.slice(0, 10) // keep results tight
      : matches.filter((m: any) => m.score >= FALLBACK_SCORE).slice(0, 8)
    
    // Get day numbers from matches (dedupe while preserving order)
    const dayNumbers: number[] = []
    const seen = new Set<number>()
    for (const m of finalMatches) {
      const d = m?.metadata?.day_number
      if (typeof d === 'number' && !seen.has(d)) {
        seen.add(d)
        dayNumbers.push(d)
      }
    }
